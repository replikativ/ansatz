...
;; CIC kernel for Clojure — Reduction machinery (WHNF).

(ns cic.kernel.reduce
  ...
  (:import [cic.kernel Name ConstantInfo ConstantInfo$RecursorRule Env]))
...
(defn- instantiate-const-type
  "Get the type of a constant with universe levels instantiated."
  [^Env env ^ConstantInfo ci levels]
  (let [subst (make-level-subst (vec (.levelParams ci)) levels)]
    ...
    (when-let [^ConstantInfo ci (env/lookup env (e/const-name head))]
      (when (.isRecursor ci)
        (let [num-params (.numParams ci)
              ...
              ^"[Lcic.kernel.ConstantInfo$RecursorRule;" rules (.rules ci)
              is-k (.isK ci)
