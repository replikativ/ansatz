...
;; Lean 4 syntax emitter — produces valid, fully explicit Lean 4 code from CIC expressions.

(ns cic.surface.lean
  "Emit valid Lean 4 syntax from CIC expressions.
    ...
    (if n
      ...
      (case (lvl/tag l)
        ...
        :param (let [pn (lvl/param-name l)]
                 (if (instance? cic.kernel.Name pn)
...
(defn emit-term
  "Emit a CIC expression as valid Lean 4 syntax.
...
(defn emit-def
  "Emit a Lean 4 `def` declaration.
   name-str is a string, type and value are CIC expressions."
  [name-str type value]
...
(defn emit-theorem
  "Emit a Lean 4 `theorem` declaration.
   name-str is a string, type and proof are CIC expressions."
  [name-str type proof]
  (let [n (escape-name name-str)
        ty (emit-term type)
        pf (emit-term proof)]
