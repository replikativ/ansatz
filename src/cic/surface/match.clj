...
(ns cic.surface.match
  "Match expression compilation: surface patterns → CIC recursor application.
  ...
  (:require [cic.kernel.expr :as e]
            ...
            [cic.kernel.tc :as tc])
  (:import [cic.kernel ConstantInfo ConstantInfo$RecursorRule Env]))
    ...
    (loop [t ctor-type fvars [] result []]
      (if (e/forall? t)
        (let [fid (+ 700000 (count fvars))
              fv (e/fvar fid)
              field-name (or (e/forall-name t) (str "x" (count fvars)))
                ...
                (let [fid (nth field-fvar-ids i)
                  ...
                  (cond-> est
                    ...
                    (update :tc update :lctx
                            red/lctx-add-local fid field-name ft))))
              est
              (range nfields))
        ;; Add IH fvars to lctx (not in user scope)
        est' (reduce
              (fn [est i]
                (let [fid (nth ih-fvar-ids i)
                      iht (nth ih-types i)]
                  (update est :tc update :lctx
                          red/lctx-add-local fid (str "ih" i) iht)))
              est'
              (range num-ih-args))
...
(defn- compile-match-term
  ...
  (let [[type-head type-args] (e/get-app-fn-args discr-type)
        ...
        ^"[Lcic.kernel.ConstantInfo$RecursorRule;" rules (.rules rec-ci)
