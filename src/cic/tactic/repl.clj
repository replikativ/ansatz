...
(ns cic.tactic.repl
  ...
  (:require [cic.kernel.expr :as e]
            ...
            [cic.tactic.sat-decide :as sat-decide-tac]
            [cic.tactic.instance :as instance]
            [cic.tactic.omega :as omega-tac]
            [cic.tactic.omega-proof :as omega-proof-tac]
            [cic.tactic.ring :as ring-tac]
            [cic.tactic.simp :as simp-tac]
...
(defn smt
  "Close a goal using SMT solving + decide verification."
  [ps]
  (decide-tac/smt ps))

(defn sat-decide
  "Close a goal using SAT solving + DRAT verification (trusted oracle)."
  [ps]
  (sat-decide-tac/sat-decide ps))

(defn sat-decide-raw
  "Close a goal using pre-built SAT clauses + DRAT verification.
   The caller ensures the encoding corresponds to the proposition."
  [ps clauses]
  (sat-decide-tac/sat-decide-raw ps clauses))
...
(defn theorem
  ...
  (let [type-expr (elab/elaborate env type-sexpr)
        ...
        ci (cic.kernel.env/mk-thm cname [] type-expr term)]
    (cic.kernel.env/add-constant env ci)
    env))
...
(defn define
  ...
  ([env def-name type-sexpr value-sexpr opts]
   (let [type-expr (elab/elaborate env type-sexpr)
         ...
         ci (cic.kernel.env/mk-def cname [] type-expr value-expr :hints hints)]
     (cic.kernel.env/add-constant env ci)
     env)))
