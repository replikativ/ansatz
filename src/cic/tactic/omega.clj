...
;; 1. Reify: CIC goal/hypotheses → linear constraints
...
;; ============================================================
;; Reification: CIC expressions → linear combinations
;; ============================================================

(defn- tactic-error! [msg data]
  (throw (ex-info (str "omega: " msg) (merge {:kind :tactic-error} data))))

;; Atom table: maps CIC expressions to variable indices
(defn- mk-atom-table []
  {:expr->idx {} :idx->expr {} :next-idx 0})

(defn- intern-atom
  "Intern a CIC expression as an atom variable. Returns [table' var-idx]."
  [table st expr]
...
(defn- reify-term
  ...
  (let [expr-whnf (#'tc/cached-whnf st expr)]
    (cond
      ...
      ;; Application — check for arithmetic ops
      (e/app? expr-whnf)
            ...
            (cond
              ...
              (let [a-whnf (#'tc/cached-whnf st (nth args 0))
                ...
                (if (and (e/lit-nat? a-whnf) (e/lit-nat? b-whnf))
                  ;; Ground: evaluate truncated subtraction
                  [table (mk-lc (max 0 (- (e/lit-nat-val a-whnf) (e/lit-nat-val b-whnf))))]
                  ;; Non-ground: intern as atom
                  (let [[table' idx] (intern-atom table st expr-whnf)]
                    [table' (lc-var idx)])))
              ...
              (and (or (= head-name nat-mul-name)
                       (= head-name hmul-name))
                   (let [mul-args (if (= head-name hmul-name)
              ...
              (let [[a b] (if (= head-name hmul-name)
                ...
                (cond
                  ...
                  ;; non-ground multiplication — intern as atom
                  :else
                  (let [[table' idx] (intern-atom table st expr-whnf)]
                    [table' (lc-var idx)])))

              ;; Nat.div a b → for ground cases evaluate, else atom
              (and (= head-name nat-div-name) (= 2 (count args)))
              (let [a-whnf (#'tc/cached-whnf st (nth args 0))
                    b-whnf (#'tc/cached-whnf st (nth args 1))]
                (if (and (e/lit-nat? a-whnf) (e/lit-nat? b-whnf))
              ...
              ;; Nat.mod a b → for ground cases evaluate, else atom
              (and (= head-name nat-mod-name) (= 2 (count args)))
              (let [a-whnf (#'tc/cached-whnf st (nth args 0))
                    b-whnf (#'tc/cached-whnf st (nth args 1))]
                (if (and (e/lit-nat? a-whnf) (e/lit-nat? b-whnf))
                  (let [bv (e/lit-nat-val b-whnf)]
                    [table (mk-lc (if (zero? bv) (e/lit-nat-val a-whnf) (mod (e/lit-nat-val a-whnf) bv)))])
                  (let [[table' idx] (intern-atom table st expr-whnf)]
                    [table' (lc-var idx)])))

              ;; HDiv.hDiv _ _ _ _ a b → for ground Nat cases evaluate, else atom
              (and (= head-name hdiv-name) (= 6 (count args)))
              ...
              ;; HMod.hMod _ _ _ _ a b → for ground Nat cases evaluate, else atom
              (and (= head-name hmod-name) (= 6 (count args)))
              (let [a-whnf (#'tc/cached-whnf st (nth args 4))
                    b-whnf (#'tc/cached-whnf st (nth args 5))]
                (if (and (e/lit-nat? a-whnf) (e/lit-nat? b-whnf))
                  (let [bv (e/lit-nat-val b-whnf)]
              ...
              ;; Unknown function — intern as atom
              :else
              (let [[table' idx] (intern-atom table st expr-whnf)]
                [table' (lc-var idx)])))))
      ...
      ;; Constant (e.g. a defined constant) — try to reduce further, else atom
      (e/const? expr-whnf)
...
;; ============================================================
;; Reification: CIC propositions → constraints
;; ============================================================
...
(def ^:private dvd-name (name/from-string "Dvd.dvd"))

(declare reify-prop)

(defn- reify-prop
  "Reify a CIC proposition into a list of constraints.
...
(defn- try-le-refl
  "Try to close a LE goal by Nat.le.refl when both sides are def-eq."
  [ps]
  (let [goal (proof/current-goal ps)]
    (when goal
      (let [st (tc/mk-tc-state (:env ps))
            ...
            [head args] (e/get-app-fn-args goal-type)]
        (when (and (e/const? head)
                   (or (= (e/const-name head) le-name)
                       (= (e/const-name head) nat-le-name)))
          ;; For Nat.le, try using the le.refl or Nat.le.refl
          nil)))))
      ...
      (or (try-rfl ps)
          ...
          (let [goal (proof/current-goal ps)
            ...
            (try
              ...
              (catch Exception _
                (tactic-error!
                 "omega: found contradiction but decide cannot certify (non-ground goal)"
                 {:goal (:type goal)}))))))))
