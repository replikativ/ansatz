...
(ns cic.tactic.sat-decide
  "The `sat-decide` tactic closes goals using SAT solving + DRAT verification.
   ...
   1. Translates CIC propositions to propositional SAT (via Tseitin encoding)
   ...
   For direct SAT encoding (bypassing CIC→SAT translation), use `sat-decide-raw`
...
;; ============================================================
;; CIC → SAT translation (propositional fragment via Tseitin)
;; ============================================================
...
(defn- sat-error! [msg data]
  (throw (ex-info (str "SAT translation: " msg) (merge {:kind :sat-error} data))))
...
(defn- term->nat
  "Try to extract a ground Nat value from a CIC term (for atomic comparisons)."
  [term]
  (when (e/lit-nat? term)
    (e/lit-nat-val term)))

(defn- prop->cnf*
  "Recursively translate a CIC proposition to CNF via Tseitin encoding.
...
(defn prop->cnf
  "Translate a CIC proposition to CNF.
...
(defn sat-decide
  ...
  (let [goal (proof/current-goal ps)
    ...
    (let [{:keys [clauses]} (prop->cnf env prop)
          ;; Solve and verify
          result (drat/solve-and-verify clauses)]
      (case (:result result)
        :unsat
        (if (:verified result)
...
(defn sat-decide-raw
  "Close the current goal using pre-built SAT clauses + DRAT verification.
   ...
   verified to correspond to the CIC proposition.
