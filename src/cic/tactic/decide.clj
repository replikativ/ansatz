...
;; Tactic layer — decide tactic, CIC→SMT-LIB translation, and smt tactic.

(ns cic.tactic.decide
  "The `decide` tactic closes goals of decidable propositions by:
   ...
   1. Translating CIC propositions to SMT-LIB
...
(defn prop->smt
  "Translate a CIC proposition to SMT-LIB s-expression (as Clojure data).
...
(defn- term->smt*
  "Translate a CIC term (Nat arithmetic) to SMT-LIB."
  [st term var-names]
...
(defn zustand-check
  "Check validity of a CIC proposition using the zustand SMT solver.
  ...
  (try
    (let [smt-sexp (prop->smt env prop)
          ...
          solver (reduce (fn [s v] (smt/declare-const s v :int))
                         (smt/make-solver)
                         vars)
          solver (smt/assert-expr solver [:not zustand-expr])
          result (smt/solve solver)]
      (case (:result result)
        :unsat  {:result :valid}
