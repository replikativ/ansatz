...
;; PSS-backed persistent store for CIC kernel.

(ns cic.export.storage
  "Durable persistence for CIC kernel state using PSS + konserve.
   ...
   Storage stack: PSS → CachedStorage (IStorage) → konserve (filestore or LMDB)."
  (:require [konserve.core :as k]
            ...
            [cic.kernel.name :as cic-name]
            [cic.kernel.level :as cic-level]
            [cic.export.parser :as parser]
            [cic.export.replay :as replay]
            [cic.export.lmdb-handlers :as lmdb-h]
            [konserve-lmdb.store :as lmdb]
            [konserve-lmdb.buffer :as lmdb-buf])
  (:import [cic.kernel Name Level Expr ConstantInfo ConstantInfo$RecursorRule Env ExprStore TypeChecker FlatStore FlatStoreWriter FlatStoreWriter$DeclProvider]
           [cic.export.lmdb_handlers CIShell]
           [org.replikativ.persistent_sorted_set PersistentSortedSet IStorage ANode Leaf Branch Settings RefType]
...
;; ============================================================
;; Fressian handlers for CIC types
;; ============================================================

(defn- write-name [^Writer writer ^Name n]
  (.writeTag writer "cic.Name" 1)
  (case (int (.tag n))
...
(def ^:private binder-info-kws
  {:default     (clojure.lang.Keyword/intern nil "default")
   :implicit    (clojure.lang.Keyword/intern nil "implicit")
   :strict-implicit (clojure.lang.Keyword/intern nil "strict-implicit")
   :inst-implicit   (clojure.lang.Keyword/intern nil "inst-implicit")})

(defn- write-expr [^Writer writer ^Expr e]
  (.writeTag writer "cic.Expr" 1)
  (let [tag (int (.tag e))]
    (case tag
...
(defn- write-recursor-rule [^Writer writer ^ConstantInfo$RecursorRule r]
  (.writeTag writer "cic.RecursorRule" 1)
...
(defn- write-ci [^Writer writer ^ConstantInfo ci]
  (.writeTag writer "cic.ConstantInfo" 1)
  (let [tag (int (.tag ci))
...
(deftype CachedStorage [store cache pending-writes freed-addresses freelist settings-atom]
  ...
  (restore [_ address]
    (if-let [cached (cache/lookup cache address)]
      ...
      (let [node (if (instance? konserve_lmdb.store.LMDBStore store)
                   ;; Direct API — values stored without {:meta :value} wrapper
                   (lmdb/get store address)
                   ;; Filestore — konserve API
...
(defn flush-writes!
  "Flush all pending writes to konserve.
   Uses multi-assoc for LMDB (single transaction), parallel batches for filestore."
  [storage]
  (let [^CachedStorage cs storage
    ...
    (when (seq writes)
      (if (instance? konserve_lmdb.store.LMDBStore kstore)
        ;; LMDB: batch write via Direct API in single transaction
        (lmdb/multi-put kstore writes)
        ;; Filestore: parallel individual writes
        (let [batch-size 64]
...
(defn open-lmdb-store
  "Open a persistent store backed by konserve-lmdb at dir-path.
   Much faster than filestore for streaming imports (single LMDB transaction per flush).
   Returns a store map with :store (LMDBStore), :storage (CachedStorage),
   and :settings-atom."
  [dir-path & {:keys [map-size] :or {map-size (* 100 1024 1024 1024)}}]
  (let [settings (Settings. 64 RefType/WEAK)
        handlers (lmdb-h/create-all-handlers settings)
        registry (lmdb-buf/create-handler-registry handlers {})
        kstore (lmdb/connect-store dir-path :map-size map-size :type-handlers registry)
        lru-cache (cache/lru-cache-factory {} :threshold 4096)
        settings-atom (atom settings)
        cached (->CachedStorage kstore lru-cache (atom []) (atom []) (atom []) settings-atom)]
    {:store kstore
     :storage cached
     :settings-atom settings-atom}))
...
(defn close-store
  ...
  (let [kstore (:store store-map)]
    (when (instance? konserve_lmdb.store.LMDBStore kstore)
      (lmdb/release-store kstore))))
...
(defn store-put
  "Put a single key-value pair to the backing store (LMDB or filestore)."
  [store k v]
  (if (instance? konserve_lmdb.store.LMDBStore store)
    (lmdb/put store k v)
    (k/assoc store k v {:sync? true})))

(defn store-get
  "Get a value from the backing store (LMDB or filestore)."
  [store k]
  (if (instance? konserve_lmdb.store.LMDBStore store)
    (lmdb/get store k)
...
(defn store-multi-put
  "Batch put key-value pairs to the backing store."
  [store entries]
  (if (instance? konserve_lmdb.store.LMDBStore store)
    (lmdb/multi-put store entries)
    ;; Filestore: parallel individual writes
    (let [batch-size 64]
...
(defn load-env
  "Load an Env from a persisted branch.
   ...
   - Flat DAG: dag-storage? without exprs-root → flat [:expr id] LMDB entries"
                        ...
                        (when result
                          (let [entry (nth result 1)]
                            (if dag?
                              ...
                              entry)))))]
      (when (:quot-enabled branch-meta)
        (.enableQuot env))
      (.setExternalLookup env lookup-fn (int (:env-count branch-meta 0)))
      env)))
...
(defn import-ndjson-streaming!
  "Parse an ndjson file and persist CI-shells + expression DAG to LMDB.
   ...
   Progress is logged to :log-file (default /tmp/cic-import.log)."
...
(defn prepare-verify
  ...
  [store-map branch-name & {:keys [log-file]
                             :or {log-file "/tmp/cic-verify.log"}}]
  (let [{:keys [storage store]} store-map
        lw (java.io.FileWriter. (str log-file) false)
        branch-meta (store-get store [:branches branch-name])]
    (when (nil? branch-meta)
    ...
    (let [decl-order (if-let [num-chunks (:decl-order-chunks branch-meta)]
          ...
          resolve-fn (fn [name-str]
                       (let [name-obj (cic-name/from-string name-str)]
                  ...
                  (run-with-large-stack
                   (fn []
                     (case (int tag)
                       ...
                       (do (.addConstant env ci) (.enableQuot env) 0)
                       (5 6 7) ;; INDUCT, CTOR, RECURSOR — check type only
                       (do (TypeChecker/checkType env ci (long fuel))
                           (.addConstant env ci) 0)
...
(defn import-to-flatstore!
  ...
  [ndjson-path out-dir & {:keys [max-count log-file]
                           :or {log-file "/tmp/cic-import.log"}}]
