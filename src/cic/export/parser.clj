...
(ns cic.export.parser
  ...
  (:import [java.util ArrayList]
           [cic.kernel Name ConstantInfo Env ExprStore]))
...
(defn- table-get [^ArrayList table idx]
  (when (and (>= idx 0) (< idx (.size table)))
    (.get table idx)))
  ...
  (let [env (env/empty-env)]
    (doseq [ci decls]
      (try
        ...
        (catch Exception ex
          (println "Warning: skipping" (.name ^cic.kernel.ConstantInfo ci) "-" (.getMessage ex)))))
    env))
