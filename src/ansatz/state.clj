...
(defonce ^{:doc "The current store's @[simp] LHS disc-tree (LHS-key → lemma-name), as an atom. nil
                 until a store's simp-keys.ndjson.gz is loaded on first demand (see ansatz.simp-index)."}
