...
(defn recall-provider
  "A candidate provider `(state, goal) → [[w name] …]` for proveo/expro/inhabito,
   ...
   recall (over-approximate, µs). Returns [] if no recall trie is loaded.

   By DEFAULT it does NOT kernel-confirm the candidates: `applyo` (which
   best-first runs to expand each candidate) already performs the exact same
   unification, so an eager `applies?` pass is redundant work AND forces the
   whole set to be confirmed up front. Instead we return a SMALL specificity
   prefix and let best-first + `applyo` confirm lazily, in measure order — the
   whole point of the search layer. (E0: eager-confirm-then-branch-40-wide
   timed out at Mathlib scale; lazy tight recall is the fix.)

   Opts (map, or a bare number = :limit for back-compat):
   - :limit   candidates returned (default 12) — kept small on purpose.
   - :confirm? eager kernel-defeq confirm before returning (default false).
   - :precap  ranked prefix that gets confirmed when :confirm? (default 400).
   - :exclude set/predicate of names to drop (e.g. the goal's own theorem)."
  ([env] (recall-provider env {}))
  ([env limit-or-opts]
   (let [{:keys [limit confirm? precap exclude]}
         (if (map? limit-or-opts) limit-or-opts {:limit limit-or-opts})
         limit (or limit 12)
         precap (or precap 400)
         excl? (cond (nil? exclude) (constantly false)
                     (set? exclude) exclude
                     :else exclude)]
     (fn [s g]
       (if-let [trie (store-recall-trie)]
         (let [gty (mvar-type s g)
               ...
               ranked (->> (dt/trie-match-scored trie (recall/query-key gty))
                           ...
                           (sort-by (comp - val))
                           (remove (fn [[nm _]] (excl? nm))))
               ranked (if confirm?
                        (filter (fn [[nm _]] (applies? env lctx gty nm)) (take precap ranked))
                        ranked)]
           (->> ranked
                (take limit)
