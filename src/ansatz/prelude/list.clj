  ...
  "The list big-operator layer of the owned prelude: a polymorphic `wsum` (fold over a WAddMonoid)
   ...
   `wsum` is authored at Lean's level of concision: a polymorphic structurally-recursive a/defn
       wsum {S} [m : WAddMonoid S] : List S → S
         | []      => m.zero
         | x :: xs => m.add x (wsum xs)
   which auto-generates the constructor-keyed equation lemmas (wsum.eq_1/eq_2) that simp consumes.
   The monoid is an INSTANCE argument (`:inst`), so it is synthesized at every call site from the
   ambient instance — the laws read like Lean (`wsum (map f xs)`, no threaded monoid), and a law over
   a `WSemiring S` instance feeds `wsum` its `WAddMonoid` via the `WSemiring.toWAddMonoid` projection.
...
(defn install!
  ...
  (ac/install!)
  (when-not (has? "wsum")
    (eval '(ansatz.core/defn wsum [S :- Type :implicit, m :- (WAddMonoid S) :inst, xs :- (List S)] S
             (match xs (List S) S
                    (nil (WAddMonoid.zero m))
                    (cons [hd tl] (WAddMonoid.add m hd (wsum tl)))))))
  ...
  (when-not (has? "wsum_map_mul_left")
    (try
      (eval '(ansatz.core/theorem wsum_map_mul_left
                                  [X :- Type, S :- Type :implicit, m :- (WSemiring S) :inst, b :- S, f :- (=> X S), xs :- (List X)]
                                  (= S
                                     (wsum
                                      (List.map X S (fn [x :- X] (WSemiring.mul m b (f x))) xs))
                                     (WSemiring.mul m b (wsum (List.map X S f xs))))
                                  (induction xs)
  ...
  (when-not (has? "wsum_map_add")
    (try
      (eval '(ansatz.core/theorem wsum_map_add
                                  [X :- Type, S :- Type, m :- (WAddMonoid S) :inst,
                                   hc :- (Std.Commutative S (WAddMonoid.add m)),
                                   f :- (=> X S), g :- (=> X S), xs :- (List X)]
                                  (= S
                                     (wsum (List.map X S (fn [x :- X] (WAddMonoid.add m (f x) (g x))) xs))
                                     (WAddMonoid.add m (wsum (List.map X S f xs)) (wsum (List.map X S g xs))))
                                  (induction xs)
                                  ...
                                  (all_goals (try (rw (op_medial S (WAddMonoid.add m)
                                                                 ...
                                                                 (f head) (g head)
                                                                 (wsum (List.map X S f tail)) (wsum (List.map X S g tail))))))))
      (catch Throwable _ nil)))
  ...
  (when-not (has? "wsum_map_const_zero")
    (try
      (eval '(ansatz.core/theorem wsum_map_const_zero
                                  [Y :- Type, S :- Type, m :- (WAddMonoid S) :inst, ys :- (List Y)]
                                  (= S (wsum (List.map Y S (fn [y :- Y] (WAddMonoid.zero m)) ys)) (WAddMonoid.zero m))
                                  (induction ys)
                                  (all_goals (simp_all [List.map_cons List.map_nil wsum.eq_1 wsum.eq_2
    ...
    (try
      (eval '(ansatz.core/theorem wsum_map_sum_comm
                                  [X :- Type, Y :- Type, S :- Type, m :- (WAddMonoid S) :inst,
                                   ...
                                   xs :- (List X), ys :- (List Y)]
                                  (= S
                                     (wsum (List.map X S (fn [x :- X] (wsum (List.map Y S (g x) ys))) xs))
                                     (wsum (List.map Y S (fn [y :- Y] (wsum (List.map X S (fn [x :- X] (g x y)) xs))) ys)))
                                  (induction xs)
                                  (all_goals (simp_all [List.map_cons List.map_nil wsum.eq_1 wsum.eq_2
                                                        (WAddMonoid.zero_add m) wsum_map_const_zero]))
                                  (all_goals (try (rw (wsum_map_add Y S hc (g head)
                                                                    (fn [y :- Y] (wsum (List.map X S (fn [x2 :- X] (g x2 y)) tail))) ys))))))
      (catch Throwable _ nil)))
  ...
  (when-not (has? "wsum_append")
    (try
      (eval '(ansatz.core/theorem wsum_append
                                  [S :- Type, m :- (WAddMonoid S) :inst, l1 :- (List S), l2 :- (List S)]
                                  (= S (wsum (HAppend.hAppend (List S) (List S) (List S)
                                                              (instHAppendOfAppend (List S) (List.instAppend S)) l1 l2))
                                     (WAddMonoid.add m (wsum l1) (wsum l2)))
                                  (induction l1)
                                  (all_goals (simp_all [List.cons_append List.nil_append wsum.eq_1 wsum.eq_2
                                                        (WAddMonoid.zero_add m)]))
                                  (all_goals (try (rw (WAddMonoid.add_assoc m head (wsum tail) (wsum l2)))))))
      (catch Throwable _ nil)))
  ...
  (when-not (has? "wsum_reverse")
    (try
      (eval '(ansatz.core/theorem wsum_reverse
                                  [S :- Type, m :- (WAddMonoid S) :inst, hc :- (Std.Commutative S (WAddMonoid.add m)), l :- (List S)]
                                  (= S (wsum (List.reverse S l)) (wsum l))
                                  (induction l)
                                  (all_goals (simp_all [List.reverse_cons List.reverse_nil wsum.eq_1 wsum.eq_2 wsum_append
                                                        (WAddMonoid.zero_add m) (WAddMonoid.add_zero m)]))
                                  (all_goals (try (rw (Std.Commutative.comm S (WAddMonoid.add m) hc head (wsum tail)))))))
      (catch Throwable _ nil)))
  ...
  (when-not (has? "wsum_map_foldl_cons")
    (try
      (eval '(ansatz.core/theorem wsum_map_foldl_cons
                                  [X :- Type, S :- Type, m :- (WAddMonoid S) :inst, hc :- (Std.Commutative S (WAddMonoid.add m)),
                                   g :- (=> X S), l :- (List X)]
                                  (= S
                                     (wsum (List.map X S g (List.foldl (List X) X (fn [acc :- (List X) x :- X] (List.cons X x acc)) (List.nil X) l)))
                                     (wsum (List.map X S g l)))
                                  (simp [foldl_cons_nil])
                                  (rw (List.map_reverse X S g l))
                                  (rw (wsum_reverse S hc (List.map X S g l)))))
      (catch Throwable _ nil)))
  ...
  (when-not (has? "wsum_flatMap")
    (try
      (eval '(ansatz.core/theorem wsum_flatMap
                                  [X :- Type, S :- Type, m :- (WAddMonoid S) :inst, f :- (=> X (List S)), l :- (List X)]
                                  (= S (wsum (List.flatMap X S f l))
                                     (wsum (List.map X S (fn [x :- X] (wsum (f x))) l)))
                                  (induction l)
  ...
  (when-not (has? "wsum_flatten")
    (try
      (eval '(ansatz.core/theorem wsum_flatten
                                  [S :- Type, m :- (WAddMonoid S) :inst, LL :- (List (List S))]
                                  (= S (wsum (List.flatten S LL))
                                     (wsum (List.map (List S) S (fn [l :- (List S)] (wsum l)) LL)))
                                  (induction LL)
  ...
  (when-not (has? "sum_filter_map")
    (try
      (eval '(ansatz.core/theorem sum_filter_map
                                  [Y :- Type, S :- Type, m :- (WAddMonoid S) :inst,
                                   p :- (=> Y Bool), h :- (=> Y S), ys :- (List Y)]
                                  (= S
                                     (wsum (List.map Y S h (List.filter Y p ys)))
                                     (wsum (List.map Y S (fn [y :- Y] (if (p y) (h y) (WAddMonoid.zero m))) ys)))
                                  (induction ys)
