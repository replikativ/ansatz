;; Surface syntax — macro-based named term builder for Ansatz expressions.

(ns ansatz.surface.term
  "Eliminates manual de Bruijn index management, name interning, and
   universe level plumbing when constructing Ansatz terms.
            ...
            (let [cname (name/from-string base-name)
              ...
              (if ci
                (let [level-params (env/ci-level-params ci)
                      ;; Default levels: try (succ zero) for each param
                      ;; This works for most Nat/Prop-level constants (Eq, And, Or, etc.)
                      ;; For higher universes, use explicit .{u} syntax
                      levels (or explicit-levels
                                 (if (empty? level-params)
                                   []
                                   (mapv (fn [_] (lvl/succ lvl/zero)) level-params)))]
...
(defn- compile-term
  "Recursively compile a Clojure s-expression into a Ansatz Expr.
  ...
  (cond
    ...
    (let [head (first sexpr)]
      (case (when (symbol? head) (str head))
        ...
        ;; (arrow Type1 Type2) or (→ Type1 Type2)
        ;; Non-dependent function type: A → B
        ;; e/arrow creates (forall _ A B), so B is under one new binder.
        ;; We compile B at depth+1 to account for the arrow's implicit binder.
        ;; This ensures outer variable references in B get correct indices.
        ("arrow" "→")
        (let [[_ a b] sexpr
              a-expr (compile-term env scope depth a)
              b-expr (compile-term env scope (inc depth) b)]
...
(defn term
  "Build a Ansatz Expr from a Clojure s-expression.
