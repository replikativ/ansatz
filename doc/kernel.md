...
The kernel data structures are intentionally close to Lean 4's `src/kernel`
objects. The table below is the side-by-side map to use when comparing the two
implementations.

| Lean 4 kernel | Ansatz kernel | Notes |
|---------------|---------------|-------|
| `expr` | `Expr` | Same expression grammar: bvar, sort, const, app, lambda, Pi, let, literals, metadata, projection, fvar. |
| `level` | `Level` | Same universe constructors: zero, succ, max, imax, param. |
| `name` | `Name` | Interned hierarchical names; pointer equality is the fast path after interning. |
| `constant_info` variants | `ConstantInfo` tags | Axiom, theorem, definition, opaque, quotient, inductive, constructor, recursor. |
| `environment` | `Env` | Immutable declaration environment; Ansatz adds staged external lookup for imported stores. |
| `local_ctx` / `local_decl` | `TypeChecker` local context, `LocalDecl` | `LocalDecl` is used by inductive/recursor generation; locals consume type annotations like Lean. |
| `type_checker` | `TypeChecker` | Type inference, definitional equality, proof irrelevance, declaration checks. |
| `whnf_core` | `Reducer.whnfCoreImpl` | Beta, zeta, delta, iota, projection, native literal reductions. |
| `equiv_manager` | `EquivManager` | Union-find for already-proven definitional equalities. |
| `inductive_decl` | `InductiveBundle` | Bundle of mutual inductive types, constructors, and recursors. |
| `add_inductive_fn` | `InductiveChecker` + `RecursorGenerator` | Same phase order; Ansatz compares generated recursors against declarations before admission. |
| `elim_nested_inductive_fn` | `NestedInductiveEliminator` | Lowers nested inductives to auxiliary ordinary inductives before bundle checking. |
| `recursor_rule` / `recursor_rules` | `ConstantInfo.RecursorRule[]` | Stored iota rules; generated by `RecursorGenerator.buildExpectedRecursorRules`. |
...
`checkInductiveBundle` follows Lean's inductive admission shape. This is the
phase map:

| Lean 4 `inductive.cpp` | Ansatz | Rule applied |
|------------------------|--------|--------------|
| `elim_nested_inductive_fn::operator()` | `NestedInductiveEliminator.eliminate` | Replace nested inductive occurrences with auxiliary inductives. |
| `add_inductive_fn::check_inductive_types` | `InductiveChecker.checkInductiveTypes` | Check headers, parameter discipline, result sorts, universe consistency. |
| `add_inductive_fn::declare_inductive_types` | `InductiveChecker.declareInductiveTypes` | Make inductive type constants visible for constructor checking. |
| `add_inductive_fn::check_constructors` | `TypeChecker.checkConstructorDeclaration` | Check constructor type, result shape, universe bounds, strict positivity. |
| `add_inductive_fn::declare_constructors` | `InductiveChecker.declareConstructors` | Make constructors visible for recursor generation. |
| `add_inductive_fn::init_elim_level` | `InductiveChecker.initElimLevel` | Decide whether elimination is restricted to `Prop`/universe zero. |
| `add_inductive_fn::init_K_target` | `InductiveChecker.initKTarget` | Detect K-like unit/predicate recursor behavior. |
| `add_inductive_fn::mk_rec_infos` | `RecursorGenerator.mkRecInfos` | Build motives, indices, major premises, and minor premise types. |
| `collect_Cs` | `RecursorGenerator.collectMotives` | Collect all motives for mutual recursors. |
| `collect_minor_premises` | `RecursorGenerator.collectMinors` | Collect all minor premises in constructor order. |
| `add_inductive_fn::mk_rec_rules` | `RecursorGenerator.buildExpectedRecursorRules` | Generate literal iota-rule RHS terms from generated locals. |
| `add_inductive_fn::declare_recursors` | `RecursorGenerator.checkAndDeclareRecursors` | Compare generated recursor type/rules with the declaration, then admit. |
| `elim_nested_inductive_result::restore_nested` | `NestedElimResult.restoreNested` | Restore auxiliary nested declarations and recursor rules. |
...
Lean-shaped recursor types and rules and compares them with the bundle
declarations before admission.

The recursor code is intentionally laid out for side-by-side comparison with
Lean 4's `src/kernel/inductive.cpp`. The important details are:

- `LocalDecl` creation calls `TypeChecker.consumeTypeAnnotations`, matching
  Lean's `consume_type_annotations` in `add_inductive_fn::mk_local_decl`.
- `buildExpectedRecursorRules` no longer templates the RHS from imported syntax.
  It rebuilds Lean's `mk_rec_rules` structure: constructor fields `b_u`,
  recursive fields `u`, IH lambdas `v`, `e_app = minor b_u v`, then
  `mk_lambda(params, Cs, minors, b_u, e_app)`.
- Motive lambdas in iota-rule RHSs are generated with default binder info,
  matching Lean's stored recursor rules. The recursor type itself has the
  generated implicit motive shape expected from Lean's `declare_recursors`.
- The Clojure inductive frontend also calls
  `TypeChecker.generateExpectedRecursorRules`, so frontend-authored inductives
  and imported inductives use the same Lean-shaped iota-rule source.

Lean constructs recursor declarations directly. Ansatz imports recursor
declarations or receives them from the local frontend, regenerates the
Lean-shaped expected type and iota rules, and admits the recursor only after the
generated data matches.

### Current alignment status

The kernel is now structurally aligned with Lean in the areas that matter for
admission:

- One inductive admission path: `TypeChecker.checkInductiveBundle` ->
  `InductiveBundleChecker` -> `InductiveChecker` -> `RecursorGenerator`.
- One recursor-rule source: `RecursorGenerator.buildExpectedRecursorRules`.
- One ordinary declaration path: `TypeChecker.checkConstant`.
- One reduction/equality engine: `Reducer` + `TypeChecker.isDefEq`.
- Store verification uses the same admission functions as in-memory checking;
  PSS and FlatStore are lookup/materialization strategies, not extra kernels.

The remaining differences from Lean are implementation-level, not intended
semantic weakenings:
...
- We do not have a formal proof that the Java implementation is equivalent to
  Lean's C++ kernel; validation is by tests, imported-store verification, and
  trace comparison.

### Tracing and fuel

Both kernels support NDJSON trace output, but the Lean 4 side requires a
patched build. We added `LEAN_KERNEL_TRACE` support to our local Lean 4 fork
(commit `55f80e10` in `../lean4`) — it is not in upstream Lean 4. Set
`LEAN_KERNEL_TRACE=/tmp/trace.ndjson` (optionally scoped with
`LEAN_KERNEL_TRACE_DECL=name`). In Ansatz, use
`TypeChecker/checkConstantTraced` with a `FileWriter`. Both emit NDJSON lines
with `{s, d, l, r, res, by}`: sequence, depth, lhs/rhs fingerprints, result,
and which step resolved it.

Each Ansatz `isDefEq` step decrements a fuel counter. If it reaches zero,
