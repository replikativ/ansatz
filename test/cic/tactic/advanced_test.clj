(ns cic.tactic.advanced-test
  ...
  (:require [clojure.test :refer [deftest testing is]]
            ...
            [cic.tactic.omega :as omega]
            [cic.tactic.omega-proof :as omega-proof]
            [cic.tactic.ring :as ring]
            [cic.tactic.simp :as simp]
...
(deftest test-omega-proof-solver-logic
  (testing "omega-proof solver correctly tracks justifications"
    (let [mk-problem @(resolve 'cic.tactic.omega-proof/mk-problem)
          add-inequality @(resolve 'cic.tactic.omega-proof/add-inequality)
          add-equality @(resolve 'cic.tactic.omega-proof/add-equality)
          run-omega-solver @(resolve 'cic.tactic.omega-proof/run-omega-solver)
          mk-atom-table @(resolve 'cic.tactic.omega-proof/mk-atom-table)]
...
(def ^:private flatstore-env
  (delay
    (let [path "/tmp/cic-flatstore-mathlib"]
      (when (.exists (java.io.File. path))
        (try
          (require '[cic.export.storage :as storage])
          (:env ((resolve 'cic.export.storage/prepare-verify-flat) path))
          (catch Exception _ nil))))))

(defn- require-flatstore-env []
  (let [env @flatstore-env]
    (when-not env
      (println "  SKIP: flatstore not available at /tmp/cic-flatstore-mathlib"))
    env))
