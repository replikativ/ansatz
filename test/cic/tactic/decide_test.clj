(ns cic.tactic.decide-test
  "Tests for instance resolution, decide tactic, CIC→SMT translation, and smt tactic."
  (:require [clojure.test :refer [deftest testing is]]
            ...
            [cic.tactic.instance :as instance]
            [cic.tactic.decide :as decide]
...
(deftest test-decide-and
  (testing "decide closes And (Eq Nat 0 0) True"
    (let [env (require-env)
          u1 (lvl/succ lvl/zero)
          nat (e/const' (name/from-string "Nat") [])
          eq00 (e/app* (e/const' (name/from-string "Eq") [u1]) nat (e/lit-nat 0) (e/lit-nat 0))
...
(deftest test-instance-index
  (testing "Instance index finds Decidable candidates"
    (let [env (require-env)
      ...
      ;; Nat.decEq should be in the list
      (is (some #(= (name/from-string "Nat.decEq") (:name %)) candidates)))))
...
;; ============================================================
;; zustand SMT solver tests
;; ============================================================

(deftest test-zustand-check-valid
  (testing "zustand-check on valid ground proposition: 2 + 3 = 5"
    (let [env (require-env)
          u1 (lvl/succ lvl/zero)
          nat (e/const' (name/from-string "Nat") [])
          two (e/lit-nat 2)
          three (e/lit-nat 3)
          five (e/lit-nat 5)
          add-2-3 (e/app* (e/const' (name/from-string "HAdd.hAdd") [lvl/zero lvl/zero lvl/zero])
                          nat nat nat
                          ;; inst — we need the instance; use Nat.add directly instead
                          )
          ;; Simpler: use prop->smt to test zustand-check with a simple Eq
          prop (e/app* (e/const' (name/from-string "Eq") [u1]) nat (e/lit-nat 5) (e/lit-nat 5))
          result (decide/zustand-check env prop)]
      (is (= :valid (:result result))))))

(deftest test-zustand-check-invalid
  (testing "zustand-check on invalid proposition: 5 = 6"
    (let [env (require-env)
          u1 (lvl/succ lvl/zero)
          nat (e/const' (name/from-string "Nat") [])
          prop (e/app* (e/const' (name/from-string "Eq") [u1]) nat (e/lit-nat 5) (e/lit-nat 6))
          result (decide/zustand-check env prop)]
      (is (= :invalid (:result result))))))

(deftest test-zustand-check-compound
  (testing "zustand-check on compound proposition: And (Eq 1 1) (Eq 2 2)"
    (let [env (require-env)
          u1 (lvl/succ lvl/zero)
          nat (e/const' (name/from-string "Nat") [])
          eq11 (e/app* (e/const' (name/from-string "Eq") [u1]) nat (e/lit-nat 1) (e/lit-nat 1))
          eq22 (e/app* (e/const' (name/from-string "Eq") [u1]) nat (e/lit-nat 2) (e/lit-nat 2))
          prop (e/app* (e/const' (name/from-string "And") []) eq11 eq22)
          result (decide/zustand-check env prop)]
      (is (= :valid (:result result))))))

(deftest test-zustand-check-not
  (testing "zustand-check on Not (Eq 0 1) — valid"
    (let [env (require-env)
          u1 (lvl/succ lvl/zero)
          nat (e/const' (name/from-string "Nat") [])
          eq01 (e/app* (e/const' (name/from-string "Eq") [u1]) nat (e/lit-nat 0) (e/lit-nat 1))
