(ns cic.kernel.typechecker-test
  ...
  (:import [cic.kernel TypeChecker Reducer Expr Env ConstantInfo Name Level]))
...
(deftest infer-forall-test
  (testing "inferType(Prop → Prop) = Sort(imax 1 1)"
    (let [tc (mk-tc)
      ...
      ;; In CIC, Π (x : A). B has type Sort(imax(level(A), level(B)))
...
(deftest is-def-eq-test
  ...
  (testing "eta equivalence for lambda: (λx. f x) =?= f"
    ;; In CIC, η-equivalence holds: λx. f x ≡ f  when x ∉ FV(f)
  ...
  (testing "Replay Nat.add_succ with checkConstantFuelStats"
    (let [f "test-data/Nat.add_succ.ndjson"]
      (when (.exists (java.io.File. f))
        (let [parse-fn (requiring-resolve 'cic.export.parser/parse-ndjson-file)
              st (parse-fn f)
              env (Env.)
              fuel 10000000
              errors (atom 0)]
