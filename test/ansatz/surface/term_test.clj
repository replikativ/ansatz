(ns ansatz.surface.term-test
  ...
  (:require [clojure.test :refer [deftest testing is]]
            ...
            [ansatz.surface.term :as ansatz]
            [ansatz.export.parser :as parser]
            [ansatz.export.replay :as replay])
  (:import [ansatz.kernel TypeChecker Env]))
...
(deftest test-forall-bound-var
  (testing "(forall [a Nat] a) — a refers to bvar 0"
    (let [env (require-init-medium)
          result (ansatz/term env '(forall [a Nat] a))]
      (is (e/forall? result))
      ;; domain is Nat
...
(deftest test-nested-forall
  (testing "(forall [a Nat] (forall [b Nat] (Eq Nat a b))) — correct de Bruijn"
    (let [env (require-init-medium)
          result (ansatz/term env '(forall [a Nat] (forall [b Nat] (Eq Nat a b))))]
      ;; Outer forall
      (is (e/forall? result))
...
(deftest test-multi-binder-forall
  (testing "(forall [a Nat, b Nat] (Eq Nat a b)) — multiple binders"
    (let [env (require-init-medium)
          result (ansatz/term env '(forall [a Nat, b Nat] (Eq Nat a b)))]
...
(deftest test-lambda
  (testing "(lam [x Prop] x) — identity function on Prop"
    (let [env (env/empty-env)
          result (ansatz/term env '(lam [x Prop] x))]
      (is (e/lam? result))
...
(deftest test-arrow
  (testing "(arrow Prop Prop) — Prop -> Prop"
    (let [env (env/empty-env)
          result (ansatz/term env '(arrow Prop Prop))
...
(deftest test-application-in-forall
  (testing "(forall [a Nat] (Eq Nat a a)) — application of Eq"
    (let [env (require-init-medium)
          result (ansatz/term env '(forall [a Nat] (Eq Nat a a)))]
      (is (e/forall? result))
  ...
  (testing "(forall [a Nat] (Eq Nat a a)) uses level params from env"
    (let [env (require-init-medium)
      ...
      (let [body (e/forall-body result)
        ...
        (is (e/const? head))
        ;; The Eq constant should have concrete level (succ zero = 1)
