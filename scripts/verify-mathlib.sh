...
# a 10 min CPU-time budget each and rewrites the checkpoints to what still fails.
#
# Environment: WORKERS (4), HEAP per worker (3g), BRANCH (mathlib), ANSATZ_STORE_DIR.
...
WORKERS="${WORKERS:-4}"; HEAP="${HEAP:-3g}"; BRANCH="${BRANCH:-mathlib}"
