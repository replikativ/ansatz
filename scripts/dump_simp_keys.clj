...
;; demand) instead of resolving+keying the whole set on every call (ansatz.simp-index).
;;
...
;; full Mathlib's ~91k @[simp] lemmas expect tens of minutes; re-run only after
...
(let [store-name (or (first *command-line-args*) "mathlib")
      ...
      names (sort (map (fn [n] (if (instance? ansatz.kernel.Name n) (name/->string n) (str n)))
